...
v0.22.0 (2026-09-18): **a picture is geometry.** The media step of Phase 5,
and the one that needed a decision: how does an asset travel? The answer
was already in the codebase — Vector In stores its paths in `node.values`
and needs no manifest — so Image In does the same: the file is read
editor-only in `buildBody`, downscaled to ≤1024px on the long side, and
stored as a data URI (PNG when small or transparent, JPEG 0.85 otherwise)
with its natural size beside it; a URL in `U` references instead. No
format bump. The kind is `{kind:'image', src, cx, cy, w, h, rot, alpha}`
and it behaves like every other kind: `toPoly` gives its frame, so bounds,
hit tests and clips work; `xformGeom` moves the frame (a conformal map
carries the picture's x axis through exactly — the ellipse's trick, learned
again when a rotate-and-scale came back at half the angle; a skew falls to
the SVD); Draw paints it and strokes the frame. What the engine never does
is load a pixel: `params/image` *declares* `{src}` on `ctx.imageList`, and
the image host — `js/images.js`, one self-contained `makeHost` serialized
into exports exactly like the audio host, gated on a compute reading the
channel — decodes it into `LM.IMG[src]`, the one registry `drawItem` reads,
and reports `{ready, w, h, sw, sh, data}` on `ctx.imageState` from a ≤256px
sampling copy (a halftone over a thousand points must not walk a 4000px
photo). `LM.IMG` serializes as `{}` so an export starts empty. **Image
Sample** (`disp/sample`) is then small: `LM.imageAt` carries the point back
through the frame into 0..1 image space and reads a byte; the node emits
color, luma brightness and alpha, and transparent-black before the picture
arrives so a Draw downstream just waits. Clusters forward both channels;
thumbnails declare through the live host so the gallery catches up on its
next paint; share links strip embedded pictures and say so (base62 is
O(n²), and no chat app wants a 60 KB link) — Save carries them. *Halftone*
is the example, and its picture is a 700-byte SVG data URI so the corpus
stays text: a Grid, one Sample, one Remap, and the dots read the plate.
Smoke pins the kind (frame, transform, draw calls with and without the
registry), the sampler on a 2×2 fake read-back, both nodes, the cluster
forward and the exporter's gate. Open: an image *paint* (pattern fill),
blend modes, and video/webcam — for which the channel is already the
right shape, since a frame is only a src that changes.
